...
public class Day7 {
    ...
    public static String runA(String input) {
        ...
        List<String> carryingPrograms = Arrays.stream(split)
                .filter(string -> string.contains("->"))
                .collect(Collectors.toList());

        List<Program> listOfPrograms = new ArrayList<>();
        Set<String> programNames = new HashSet<>();
        Set<String> carriedPrograms = new HashSet<>();

        carryingPrograms.stream()
                .forEach(string -> {
                    Program program = new Program(string);
                    programNames.add(program.name);
                    carriedPrograms.addAll(program.carrying);
                });

        programNames.removeAll(carriedPrograms);


        // DON'T FORGET TO ADD THE ANSWER
        return programNames.stream().findAny().get();
    }
    ...
    static class Program {
        ...
        Program(String string) {
            ...
            this.weight = Integer.parseInt(nameAndWeight[1].substring(0, nameAndWeight[1].length()-1));

            if(split.length > 1) {
                String[] carrying = split[1].split(", ");
                this.carrying = new HashSet<>(Arrays.asList(carrying));
            }
        }

        int getTotalWeight(Map<String, Program> programMap) {
            if(carrying.size() == 0) {
                return weight;
            }
            else {
                ...
                carrying.stream()
                        .forEach(programName -> {
                            int weightlifter = programMap.get(programName).getTotalWeight(programMap);
                            weightMap.put(programName, weightlifter);
                        });

                isWeightInBalance(weightMap, programMap);
               int carriedWeight = weightMap.values().stream().reduce(0, (a, b) -> a + b);
                totalWeight = weight + carriedWeight;
                return totalWeight ;
            }
        }

        private boolean isWeightInBalance(Map<String, Integer> weightMap, Map<String, Program> programMap) {
            HashSet<Integer> objects = new HashSet<>(weightMap.values());
            if(objects.size() == 1) {
                return true;
            }
            int neededWeight = objects.stream().findAny().get();
            for(Map.Entry<String,Integer> entry : weightMap.entrySet()) {
                if(entry.getValue() != neededWeight) {
                    Program faultyProgram = programMap.get(entry.getKey());
                    int changeWeight = faultyProgram.totalWeight - neededWeight;
                    int requiredWeight = faultyProgram.weight - changeWeight;
                    System.out.println(requiredWeight);
                }
            }
    ...
    public static String runB(String input) {
        String answer = "No answer yet";
        String[] programCallOuts = input.split("\n");
          List<Program> listOfPrograms = new ArrayList<>();

        Map<String, Program> programMap = new HashMap<>();

        Arrays.stream(programCallOuts)
                .forEach(string -> {
                    Program program = new Program(string);
                    programMap.put(program.name, program);
                });

        String bottomProgram = runA(input);
        programMap.get(bottomProgram).getTotalWeight(programMap);
