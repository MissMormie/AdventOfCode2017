...
public class Day4 {
    ...
    public static int runA(String input) {
        ...
        int answer = (int) Arrays.stream(rows).filter(Day4::isValidPassPhrase).count();
    ...
    public static boolean isValidPassPhrase(String passPhrase) {
        String[] s = passPhrase.split(" ");
        Set<String> wordsInPassPhrase = new HashSet<>(Arrays.asList(s));
        return wordsInPassPhrase.size() == s.length;
    }

    public static boolean isValidAnagramPassPhrase(String passPhrase) {
        String[] s = passPhrase.split(" ");
        StringBuilder sb = new StringBuilder();
        Arrays.stream(s).forEach(passWord -> sb.append(StringHelper.sortCharsInStringAlphabetically(passWord) + " "));
        return isValidPassPhrase(sb.toString());
    }
    ...
    public static int runB(String input) {
        ...
        int answer = (int) Arrays.stream(rows).filter(Day4::isValidAnagramPassPhrase).count();
