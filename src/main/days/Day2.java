...
public class Day2 {
    ...
    public static String runA(String input) {
        ...
        int intAnswer = 0;
        for(String row : rows) {
            String[] stringNums = row.split("\t");
            Set<Integer> numbersInRow = Arrays.stream(stringNums)
                    .map(stringNum -> Integer.parseInt(stringNum)).collect(Collectors.toSet());
            Integer min = Collections.min(numbersInRow);
            Integer max = Collections.max(numbersInRow);
            intAnswer += max - min;
        }

        String answer = "" + intAnswer;
        System.out.println("answer A: " + answer);
        return answer;
    }

    private static int publicIntAnswer;
    public static String runB(String input) {
        ...
        for(String row : rows) {
            ...
            numbersInRow.stream()
                    .filter(num -> numbersInRow.stream()
                            .anyMatch(checkNum -> num % checkNum == 0 && checkNum != num))
                    .findFirst()
                    .ifPresent(num -> numbersInRow.stream()
                            .forEach(checkNum -> {if(num % checkNum == 0 && checkNum != num); updateAnswer(num/checkNum); }));
        }

        String answer = "" + publicIntAnswer;
        System.out.println("answer B: " + answer);
        return answer;
    }

    public static  void updateAnswer(int answer1) {
        publicIntAnswer += answer1;
    }
    ...
    public static String getInput() {
        return "1919\t2959\t82\t507\t3219\t239\t3494\t1440\t3107\t259\t3544\t683\t207\t562\t276\t2963\n" +
